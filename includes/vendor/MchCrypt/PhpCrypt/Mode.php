...
abstract class PhpCrypt_Mode
{
	 ...
	 * PHP_Crypt::PAD_ZERO
	 */
	private $padding = PhpCrypt::PAD_ZERO;
	 ...
	 */
	public function createIV($src = PhpCrypt::RAND)
	{
		// if the mode does not use an IV, lets not waste time
		if(!$this->requiresIV())
			return false;

		$iv = PhpCrypt_Core::randBytes($src, $this->block_size);
		return $this->IV($iv);
	}
	...
	protected function pad(&$str)
	{
		...
		PhpCrypt_Padding::pad($str, $num, $this->padding);
		return true;
	}
	...
	{
		PhpCrypt_Padding::strip($str, $this->padding);
		return true;
	}
