...
//		}, $parts);
		...
		$parts = array_map(array( __CLASS__, 'hex2BinMap'), $parts);
		
		return implode("", $parts);
	}
	public static function  hex2BinMap($v)
	{
		$v = base_convert($v, 16, 2);
		return str_pad($v, 8, "0", STR_PAD_LEFT);		
	}
	...
	public static function hex2Str($hex)
	{
		...
		$parts = str_split($hex, 2);
//		$parts = array_map(function($v) {
//				return chr(PhpCrypt_Core::hex2Dec($v));
//		}, $parts);

		$parts = array_map(array( __CLASS__, 'hex2StrMap'), $parts);
		return implode("", $parts);
	}
	public static function hex2StrMap($v)
	{
		return chr(PhpCrypt_Core::hex2Dec($v));
	}
...
//		}, $parts);
		...
		$parts = array_map(array( __CLASS__, 'bin2HexMap'), $parts);
		
		return implode("", $parts);
	}
	public static function bin2HexMap($v)
	{
		$v = str_pad($v, 8, "0", STR_PAD_LEFT);
		$v = dechex(bindec($v));
		return str_pad($v, 2, "0", STR_PAD_LEFT);
...
//		$parts = array_map(function($v) {
//			return PhpCrypt_Core::hex2Bin($v);
//		}, $parts);
		
		$parts = array_map(array( __CLASS__, 'str2BinMap'), $parts);
		return implode("", $parts);
	}
	public static function str2BinMap($v)
	{
		return PhpCrypt_Core::hex2Bin($v);
	}
...
//		});
		...
		array_walk($hex, array('self::str2BinMap'));
	...
	public static function xorHexWalk(&$val, $i)
	{
		if(strlen($val) % 2)
			$val = "0".$val;		
	}
	 ...
	 */
	public static function randBytes($src = PhpCrypt::RAND, $byte_len = PhpCrypt::RAND_DEFAULT_SZ)
	{
		$bytes = "";
		$err_msg = "";

		if($src == PhpCrypt::RAND_DEV_RAND)
		{
			if(file_exists(PhpCrypt::RAND_DEV_RAND))
				$bytes = file_get_contents(PhpCrypt::RAND_DEV_RAND, false, null, 0, $byte_len);
			else
				$err_msg = PhpCrypt::RAND_DEV_RAND." not found";
		}
		else if($src == PhpCrypt::RAND_DEV_URAND)
		{
			if(file_exists(PhpCrypt::RAND_DEV_URAND))
				$bytes = file_get_contents(PhpCrypt::RAND_DEV_URAND, false, null, 0, $byte_len);
			else
				$err_msg = PhpCrypt::RAND_DEV_URAND." not found";
		}
		else if($src == PhpCrypt::RAND_WIN_COM)
		{
