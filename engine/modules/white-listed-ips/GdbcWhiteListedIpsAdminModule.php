...
final class GdbcWhiteListedIpsAdminModule extends GdbcBaseAdminModule
{
	CONST OPTION_WHITE_LISTED_IPS = 'WhiteListedIps';
	...
	public function getDefaultOptions()
	{
		...
		$arrDefaultSettingOptions = array(
			...
			self::OPTION_WHITE_LISTED_IPS => array(
				'Id'         => 1,
				'Value'      => null,
				'LabelText'  => null,
	...
	public  function validateModuleSettingsFields($arrSettingOptions)
	{
		...
		$arrAlreadySavedIps = (array)$this->getOption(self::OPTION_WHITE_LISTED_IPS);
		
		$action = 'add';
		if(!empty($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS]) && strpos($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS], 'remove-') !== false)
		{
			$action = 'remove';
			$arrSettingOptions[self::OPTION_WHITE_LISTED_IPS] = str_replace('remove-', '', $arrSettingOptions[self::OPTION_WHITE_LISTED_IPS]);
		}
		
		$preparedData = empty($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS]) ? array() : GdbcIPUtils::getFormattedIpRangeForDb($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS]);
		...
		if('remove' === $action && !empty($preparedData[$ipVersion]))
		{
			...
			$arrSettingOptions[self::OPTION_WHITE_LISTED_IPS] = $arrAlreadySavedIps;
		...
		{
			if (GdbcIPUtils::isIpWhiteListed($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS]))
			{
				$this->registerErrorMessage(sprintf(__("The IP  %s is already white-listed!", GoodByeCaptcha::PLUGIN_SLUG), esc_html($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS])));
				$arrSettingOptions[self::OPTION_WHITE_LISTED_IPS] = $arrAlreadySavedIps;
		...
		if(1 !== $preparedData[$ipVersion][1])
		{
			foreach($arrAlreadySavedIps[$ipVersion] as $minValue => $maxValue)
			{
				...
				{
					$this->registerErrorMessage(sprintf(__("The IP Range  %s is already white-listed!", GoodByeCaptcha::PLUGIN_SLUG), esc_html($arrSettingOptions[self::OPTION_WHITE_LISTED_IPS])));
					$arrSettingOptions[self::OPTION_WHITE_LISTED_IPS] = $arrAlreadySavedIps;
	...
	public function registerWhiteListedIp($ipAddress)
	{
		...
		$arrAlreadySavedIps = $this->getOption(self::OPTION_WHITE_LISTED_IPS);
		...
		return $this->saveOption(self::OPTION_WHITE_LISTED_IPS, $arrAlreadySavedIps);

	}

	public function unRegisterWhiteListedIp($ipAddress)
	{
		$ipVersion = MchGdbcIPUtils::getIpAddressVersion($ipAddress);
